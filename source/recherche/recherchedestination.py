...
class RechercheDestination(AbstractRecherche):
    ...
    def recherche( date, origine, alerter : bool = False, eligible = "OUI"):
        ...
        DAOHistorique.remplir(type='destination', date=date, origine=origine,eligible=eligible,alerter=alerter,destination='none')
        ...
        DAOTrajet.DAOTrajet(url=str(url))
        dict = rq.get(url)
        dict2=dict.json()
